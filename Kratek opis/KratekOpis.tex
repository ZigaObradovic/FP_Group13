...
\begin{enumerate}
  ...
  \item $Ln\_graph(n)$: Funkcija, ki sprejme število vozlišč $n$ in vrne \textbf{graf $L_n$}. Ideja funkcije je, da definiramo tri različne gradnike, ki jih sestavimo skupaj gleda na število $n$.
  \begin{figure}[h!]
    \centering
    \includegraphics[width=0.5\textwidth]{Slike/gradniki.png}
    \caption{Gradniki grafa $L_n$}
    \label{fig:gradniki}
\end{figure}
  \item $cubic\_graphs(n)$: Funkcija, ki sprejme število vozlišč $n$ in generira vse možne \textbf{kubične grafe} na $n$ vozliščih (za sode $n \geq 4$).
\end{enumerate}

\noindent Z uporabo teh treh funkcij želiva pokriti predvsem prvi del naloge, ki je dokazati domnevo, da ima graf $L_n$ najmanjši subpath number za majhne $n$ in poiskati kakšen protiprimer za večje $n$.
...
Ker je vseh kubičnih grfov $41301$ za $n = 18$, za $n = 20$ pa že kar $510489$, bo težko primerjati subpath number čisto vseh grafov za $n \geq 20$. Zato bova uporabila \textbf{simulated annealing}, ki je metahevristična metoda, kjer naključno spreminjamo povezave prvotnega grafa v upanju, da najdemo ustreznejšo rešitev. Še boljše pa bi bilo, če nama uspe skonstruirati tako zaporedje grafov, da bo njihov subpath
number manjši od $L_n$ grafov za vsak $n$. Tako bi subpath number lahko primerjala za veliko večje $n$, saj bi morala za vsak $n$ izračunati subpath number le za dva grafa. Od tod sledi hipoteza: 
\section{Hipoteza}
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.24\textwidth]{Slike/graf_n28.png}
\includegraphics[width=0.24\textwidth]{Slike/graf_n30.png}
\includegraphics[width=0.24\textwidth]{Slike/graf_n32.png}
\caption{Primeri \textit{boljših} grafov za $n = 28$, $30$ in $32$}
\end{figure}

\noindent Najina naloga sedaj je preveriti resničnost te hipoteze in morda poiskati kakšne grafe s še nižjim subpath number od teh.
